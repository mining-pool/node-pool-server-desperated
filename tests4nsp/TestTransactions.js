const assert = require("assert")
const tx = require("../lib/transactions")
const utils = require("../lib/util")

describe("Transactions", function() {
    it("CreateGeneration", async function() {
        const rpcData = {
          "capabilities": [
            "proposal"
          ],
          "version": 536870912,
          "rules": [
            "csv",
            "segwit"
          ],
          "vbavailable": {
          },
          "vbrequired": 0,
          "previousblockhash": "b83b698bed0897ac94819041aec857d1a26a567bf7bf046d60849d5ccf24155e",
          "transactions": [
            {
              "data": "",
              "txid": "f9b8b0bdd0dc38b2a707faf89acf064f543c3a88d39f54fb126cbd084ffb5ed9",
              "hash": "f9b8b0bdd0dc38b2a707faf89acf064f543c3a88d39f54fb126cbd084ffb5ed9",
              "depends": [
              ],
              "fee": 6450,
              "sigops": 8,
              "weight": 1028
            },
            {
              "data": "",
              "txid": "620c724890f76b802714d786d5d3fe13a89106d81e93b74c4eafd6dc04179f37",
              "hash": "620c724890f76b802714d786d5d3fe13a89106d81e93b74c4eafd6dc04179f37",
              "depends": [
              ],
              "fee": 3493,
              "sigops": 8,
              "weight": 1328
            }
          ],
          "coinbaseaux": {
            "flags": ""
          },
          "coinbasevalue": 2500009943,
          "longpollid": "b83b698bed0897ac94819041aec857d1a26a567bf7bf046d60849d5ccf24155e19620",
          "target": "00000000a9490000000000000000000000000000000000000000000000000000",
          "mintime": 1581747579,
          "mutable": [
            "time",
            "transactions",
            "prevblock"
          ],
          "noncerange": "00000000ffffffff",
          "sigoplimit": 80000,
          "sizelimit": 4000000,
          "weightlimit": 4000000,
          "curtime": 1581749398,
          "bits": "1d00a949",
          "height": 1369986
        }
        const g = tx.CreateGeneration(rpcData, utils.addressToScript("QPxrDq3sorCk8DWaYX2GeCkxoePhm1asyY"), Buffer.from("f000000ff111111f", "hex"), "POW", true, {})
        console.log(g[0].toString("hex"))
        console.log(g[1].toString("hex"))
    });
});



describe("Transactions", function() {
  it("CreateGeneration", async function() {
      const rpcData = {
        "capabilities": [
          "proposal"
        ],
        "version": 536870912,
        "rules": [
          "csv",
          "segwit"
        ],
        "vbavailable": {
        },
        "vbrequired": 0,
        "previousblockhash": "b83b698bed0897ac94819041aec857d1a26a567bf7bf046d60849d5ccf24155e",
        "transactions": [
          {
            "data": "",
            "txid": "f9b8b0bdd0dc38b2a707faf89acf064f543c3a88d39f54fb126cbd084ffb5ed9",
            "hash": "f9b8b0bdd0dc38b2a707faf89acf064f543c3a88d39f54fb126cbd084ffb5ed9",
            "depends": [
            ],
            "fee": 6450,
            "sigops": 8,
            "weight": 1028
          },
          {
            "data": "",
            "txid": "620c724890f76b802714d786d5d3fe13a89106d81e93b74c4eafd6dc04179f37",
            "hash": "620c724890f76b802714d786d5d3fe13a89106d81e93b74c4eafd6dc04179f37",
            "depends": [
            ],
            "fee": 3493,
            "sigops": 8,
            "weight": 1328
          }
        ],
        "coinbaseaux": {
          "flags": ""
        },
        "coinbasevalue": 2500009943,
        "longpollid": "b83b698bed0897ac94819041aec857d1a26a567bf7bf046d60849d5ccf24155e19620",
        "target": "00000000a9490000000000000000000000000000000000000000000000000000",
        "mintime": 1581747579,
        "mutable": [
          "time",
          "transactions",
          "prevblock"
        ],
        "noncerange": "00000000ffffffff",
        "sigoplimit": 80000,
        "sizelimit": 4000000,
        "weightlimit": 4000000,
        "curtime": 1581749398,
        "bits": "1d00a949",
        "height": 1369986
      }
      const g = tx.CreateGeneration(rpcData, utils.addressToScript("QPxrDq3sorCk8DWaYX2GeCkxoePhm1asyY"), Buffer.from("f000000ff111111f", "hex"), "POW", true, {})
      console.log(g[0].toString("hex"))
      console.log(g[1].toString("hex"))
  });

  it("GenerateOutputTransactions", async function() {
      const rpcData = {
        "capabilities": [
          "proposal"
        ],
        "version": 536870912,
        "rules": [
          "csv",
          "segwit"
        ],
        "vbavailable": {
        },
        "vbrequired": 0,
        "previousblockhash": "b83b698bed0897ac94819041aec857d1a26a567bf7bf046d60849d5ccf24155e",
        "transactions": [
          {
            "data": "",
            "txid": "f9b8b0bdd0dc38b2a707faf89acf064f543c3a88d39f54fb126cbd084ffb5ed9",
            "hash": "f9b8b0bdd0dc38b2a707faf89acf064f543c3a88d39f54fb126cbd084ffb5ed9",
            "depends": [
            ],
            "fee": 6450,
            "sigops": 8,
            "weight": 1028
          },
          {
            "data": "",
            "txid": "620c724890f76b802714d786d5d3fe13a89106d81e93b74c4eafd6dc04179f37",
            "hash": "620c724890f76b802714d786d5d3fe13a89106d81e93b74c4eafd6dc04179f37",
            "depends": [
            ],
            "fee": 3493,
            "sigops": 8,
            "weight": 1328
          }
        ],
        "coinbaseaux": {
          "flags": ""
        },
        "coinbasevalue": 2500009943,
        "longpollid": "b83b698bed0897ac94819041aec857d1a26a567bf7bf046d60849d5ccf24155e19620",
        "target": "00000000a9490000000000000000000000000000000000000000000000000000",
        "mintime": 1581747579,
        "mutable": [
          "time",
          "transactions",
          "prevblock"
        ],
        "noncerange": "00000000ffffffff",
        "sigoplimit": 80000,
        "sizelimit": 4000000,
        "weightlimit": 4000000,
        "curtime": 1581749398,
        "bits": "1d00a949",
        "height": 1369986
      }
      const g = tx.generateOutputTransactions(utils.addressToScript("QPxrDq3sorCk8DWaYX2GeCkxoePhm1asyY"), {}, rpcData)
      console.log(g.toString("hex"))
  });
});
